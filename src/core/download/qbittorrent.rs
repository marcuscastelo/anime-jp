...
use super::downloader::{RawDownloader, FileDownloader, Destination};
...
impl FileDownloader for QBitTorrentDownloader {
    ...
    fn download_indexer_to_file(&self, indexer: &Indexer, dest: &Destination) -> Result<(), Box<dyn Error>> {
        todo!();
    }
    
    fn download_indexers_to_file(&self, indexers: &[Indexer], dest: &Destination) -> Result<(), Box<dyn Error>> {
        todo!();
    }
}
